...
async function generatePredictionRun(userId,targetYear=2027){
  ...
  const samples=await db.query(`SELECT q.id,q.discipline,q.topic,q.statement,q.style_notes,e.ano,a.difficulty,a.estimated_minutes FROM questions q JOIN exams e ON e.id=q.exam_id LEFT JOIN question_ai_analysis a ON a.question_id=q.id WHERE q.source!='ai_practice' AND (e.user_id IS NULL OR e.user_id=$1) ORDER BY e.ano DESC,q.number LIMIT 180`,[userId]);
  ...
  for(const item of raw){
    ...
    validated.push({...item,rank:validated.length+1,discipline:p.discipline,topic:p.topic,confidence,estimated_minutes:minutes,difficulty,evidence_years:[...new Set(years)],evidence_question_ids:[...new Set(ids)]});
...
async function getLatestPrediction(userId,targetYear=2027){
  ...
  const items=await db.query(`SELECT * FROM predicted_question_blueprints WHERE run_id=$1 ORDER BY rank`,[run.id]);
  return {run,items};
}

module.exports={calibrateDifficulty,getDifficultySummary,getRepeatedByTopic,generatePredictionRun,getLatestPrediction};
