...
function accessState(user) {
  ...
  if (user.access_until) {
    const end = new Date(`${String(user.access_until).slice(0, 10)}T23:59:59`);
    if (end.getTime() < Date.now()) return { allowed: false, reason: 'expired' };
  }
  return { allowed: true };
}
...
router.get('/login', (req, res) => {
  res.render('login', { error: null, success: req.query.cadastro === 'pendente' ? 'Cadastro solicitado com sucesso. Aguarde a aprovação do administrador para acessar a plataforma.' : null, mode: 'login' });
});
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    if (!user || !verifyPassword(password, user.password_hash)) {
      return res.status(401).render('login', { error: 'E-mail ou senha inválidos.', success: null, mode: 'login' });
    }

    const state = accessState(user);
    if (!state.allowed) {
      return res.status(403).render('login', { error: accessMessage(state.reason), success: null, mode: 'login' });
    }
...
router.get('/cadastro', (req, res) => {
  res.render('login', { error: null, success: null, mode: 'register' });
});
...
router.post('/cadastro', async (req, res) => {
  ...
  if (name.length < 3 || !email.includes('@') || password.length < 8) {
    return res.status(400).render('login', {
      error: 'Informe seu nome, um e-mail válido e uma senha com pelo menos 8 caracteres.',
      success: null,
      mode: 'register'
    });
  }
  ...
  try {
    ...
    res.status(400).render('login', { error: 'Este e-mail já está cadastrado.', success: null, mode: 'register' });
  }
});
